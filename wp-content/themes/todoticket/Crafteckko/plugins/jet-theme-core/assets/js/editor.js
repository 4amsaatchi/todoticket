(function( $ ) {
			...
			self.JetSearchView = window.elementor.modules.controls.BaseData.extend( {
				...
				onReady: function() {
					...
					this.ui.select.select2( {
						ajax: {
							url: ajaxurl + '?action=' + this.model.attributes.action,
							dataType: 'json'
						},
	...
	JetThemeEditor = {
		...
		onPreviewLoaded: function() {
			...
			window.elementor.$previewContents.find( '.jet-edit-template-part' ).each( function() {

				var $this        = $( this ),
					$elContainer = $this.prev( '.elementor' );

				$elContainer.addClass( 'jet-has-edit' );
				$this.detach().appendTo( $elContainer );
