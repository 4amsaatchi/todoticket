...
class Jet_CW_Plugin_Changelog {
	...
	public function delete_transients( $data ) {
		...
		if ( ! empty( $this->api['plugin_file'] ) ) {
			$plugin = $this->api['plugin_file'];
		} else {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		if ( $plugin === $plugin_file ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'jet-cw' ), $this->api['name'] ) ),
				esc_attr( $this->api['name'] ),
				esc_html__( 'View details', 'jet-cw' )
			);
...
if ( ! function_exists( 'jet_cw_plugin_changelog' ) ) {
	function jet_cw_plugin_changelog() {
		return new Jet_CW_Plugin_Changelog();
	}
}
