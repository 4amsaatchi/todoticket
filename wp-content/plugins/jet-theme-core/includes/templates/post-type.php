...
if ( ! class_exists( 'Jet_Theme_Core_Templates_Post_Type' ) ) {
	...
	class Jet_Theme_Core_Templates_Post_Type {
		...
		/**
		 * Print template part by ID
		 *
		 * @return void
		 */
		public function print_template_part( $id, $foramt = '%s', $context = 'header' ) {

			$elementor = Elementor\Plugin::instance();
			$content   = $elementor->frontend->get_builder_content_for_display( $id );

			switch ( $context ) {
				case 'footer':
					$context_label = esc_html__( 'Footer', 'jet-theme-core' );
					break;

				default:
					$context_label = esc_html__( 'Header', 'jet-theme-core' );
					break;
			}

			if ( isset( $_GET['elementor-preview'] ) ) {

				$label = sprintf( esc_html__( 'Edit %s', 'jet-theme-core' ), $context_label );

				$content .= sprintf(
					'<a href="%1$s" target="_blank" class="elementor-clickable jet-edit-template-part"><span class="jet-edit-template-part__inner">%2$s</span></a>',
					Elementor\Utils::get_edit_link( $id ),
					$label
				);
			}
